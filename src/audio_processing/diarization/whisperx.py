    ...
    def __init__(self):
        try:
            ...
            if local_model_path.exists():
                ...
                logger.info("[DIARIZATION] Prioritizing Pyannote Community-1 (4.0)")
                try:
                    self.diarization_pipeline = Pipeline.from_pretrained(
                        "pyannote/speaker-diarization-community-1",
                        use_auth_token=hf_token
                    )
                    logger.info("✅ Using Pyannote Community-1 (4.0)")
                except Exception as e:
                    logger.warning(f"Community-1 not available ({e}), falling back to 3.1")
                    self.diarization_pipeline = Pipeline.from_pretrained(
