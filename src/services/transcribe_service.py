...
from src.services.hallucination_filter import HallucinationFilter, filter_hallucinations
...
def transcribe_audio(
    ...
    try:
        ...
        # ===== APPLY HALLUCINATION FILTER (from cherry_core) =====
        # This removes known hallucinations and deloops repeated words/phrases
        # Research shows 67% reduction in errors when combined with VAD
        original_len = len(transcript)
        transcript = filter_hallucinations(transcript, language="vi")

        # Also filter segments text
        if segments:
            segments = HallucinationFilter.filter_segments(segments, language="vi")

        if len(transcript) < original_len:
            logger.info(f"[HALLUCINATION_FILTER] Removed {original_len - len(transcript)} chars of hallucinations/loops")
